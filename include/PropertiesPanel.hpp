...
class RefPointPicker;
}

namespace Curvz {

class Canvas; // forward — full include would be circular; only needed to read
              // selection()
...
public:
  ...
  const std::map<std::string, bool> &section_open_state() const {
    return m_section_open;
  }
  void set_section_open_state(const std::map<std::string, bool> &state) {
  ...
  void
  sync_selected_pivot(); // s205 m1 — live-track Canvas pivot state in the
  ...
  using RequestNodeEditSignal =
      sigc::signal<void(int, double, double, double, double, double, double)>;
  ...
  using GuideSelectedSignal =
      sigc::signal<void(SceneNode *)>; // kept for compat
  ...
  RequestNodeEditSignal &signal_request_node_edit() {
    return m_sig_request_node_edit;
  }
  RequestReverseSignal &signal_request_reverse() {
    return m_sig_request_reverse;
  }
  RequestOpenAtNodeSignal &signal_request_open_at_node() {
    return m_sig_request_open_at_node;
  }
  RequestSplitSignal &signal_request_split() { return m_sig_request_split; }
  RequestNodeTypeChangeSignal &signal_request_node_type_change() {
    return m_sig_request_node_type_change;
  }
  ...
  RequestReleaseBlendSignal &signal_request_release_blend() {
    return m_sig_request_release_blend;
  }
  ...
  RequestReleaseWarpSignal &signal_request_release_warp() {
    return m_sig_request_release_warp;
  }
  RequestFlattenWarpSignal &signal_request_flatten_warp() {
    return m_sig_request_flatten_warp;
  }
  DocRenamedSignal &signal_doc_renamed() { return m_sig_doc_renamed; }
  RequestCanvasFocusSignal &signal_request_canvas_focus() {
    return m_sig_request_canvas_focus;
  }
  ...
  GuideLayerChangedSignal &signal_guide_layer_changed() {
    return m_sig_guide_layer_changed;
  }
  GuideConstructRequestedSignal &signal_guide_construct_requested() {
    return m_sig_guide_construct_requested;
  }
  ...
  using RequestGradientEditSignal = sigc::signal<void(
      FillStyle /*current*/,
  ...
  bool current_node_type_matches(SceneNode *obj, int node_idx) const {
    if (!obj || !obj->path)
      return false;
    if (node_idx < 0 || node_idx >= (int)obj->path->nodes.size())
      return false;
...
private:
  ...
  void build_canvas_section(std::shared_ptr<CanvasModel> cm,
                            Gtk::Box *parent = nullptr);
  ...
  // went internal-only.
  void build_canvas_colours_section(CurvzDocument *doc,
                                    Gtk::Box *parent = nullptr);
  ...
  void build_app_appearance_section(CurvzProject *project,
                                    Gtk::Box *parent = nullptr);
  ...
  void build_node_section(SceneNode *obj, int node_idx,
                          Gtk::Box *parent = nullptr);
  ...
  void broadcast_appearance_to_siblings(SceneNode *primary, bool fill_changed,
                                        bool stroke_changed);
  ...
  Canvas *m_canvas_widget =
      nullptr; // S58f: multi-select broadcast for Appearance
  SceneNode *m_current = nullptr;
  // Selection section — CurvzSpinButton pointers updated by
  // build_selection_section, used by sync_selection for lightweight in-place
  // updates during drag
  ...
  // Grid section — CurvzSpinButton pointers; refresh_units() called on unit
  // change
  ...
  // Margin section — CurvzSpinButton pointers; refresh_units() called on unit
  // change

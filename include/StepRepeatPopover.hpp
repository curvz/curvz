...
// ── StepRepeatPopover
// ───────────────────────────────────────────────────────── s154 m2: was
// StepRepeatDialog (Gtk::Window). Same form, same callbacks, same Result shape
// — relocated into a popover anchored to the SnR toolbar button so the
// toolbar's right-click can summon it without a separate top-level window.
//
...
public:
  ...
  void show(Gtk::Widget &anchor, const CanvasModel *model, double bbox_cx,
            double bbox_cy, double bbox_w, double bbox_h,
            PivotChangeCb pivot_cb, ApplyCb apply_cb);
  ...
  // first-show defaults.
  void apply_with_last(const CanvasModel *model, double bbox_cx, double bbox_cy,
                       ApplyCb apply_cb);
...
private:
  ...
  void on_preview_draw(const Cairo::RefPtr<Cairo::Context> &cr, int width,
                       int height);

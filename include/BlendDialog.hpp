...
public:
  ...
  // outside the dialog (MainWindow orchestrates
  // Canvas::equalize_blend_sources). The callback returns the resulting node
  // count on both sides so the dialog can refresh its banner state.
  ...
  //   is first toggled on.
  void show(Gtk::Window &parent, const CanvasModel *model, int a_count,

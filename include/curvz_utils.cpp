...
#include <glibmm/main.h>         // signal_idle for deferred focus grab
        ...
        on_finished)
{
    ...
    // Detail row — read-only Entry so the text is selectable + copyable
    // (lesson from the s125 m1j Image Info dialog: wrapping Labels cause
    // measure-loop warnings on hover; Entries don't).
    if (!detail.empty()) {
        auto* ent_detail = Gtk::make_managed<Gtk::Entry>();
        ent_detail->set_text(detail);
        ent_detail->set_editable(false);
        ent_detail->set_can_focus(true);
        ent_detail->set_hexpand(true);
        ent_detail->set_width_chars(48);
        ent_detail->add_css_class("flat");
        ent_detail->add_css_class("curvz-alert-detail");
        curvz::utils::set_name(ent_detail, "dlg_alert_detail",
                               "curvz_alert_detail_entry");
        outer->append(*ent_detail);
    }
    ...
    // Esc on transient windows by default — wire it explicitly.
    ...
    // On hide: invoke callback with the result, then delete. signal_hide
    // fires for both button clicks (which set fired=true) and X button /
    // Esc that didn't go through a button — the latter case sees
    // fired=false and reports cancel_button (or -1) as the result.
    dlg->signal_hide().connect(
        [dlg, chosen, fired, cancel_button, on_finished, collect_values]() {
            if (!*fired) {
                // X-button path — treat as cancel.
    ...
    // Defer initial focus through signal_idle (same lesson as the m1h
    // Image Info dialog — selectable widgets claim focus during the first
    // arrange pass and override our grab_focus). Default button gets
    // focus if defined; otherwise the first focusable widget.
    if (default_btn_ptr) {
        dlg->set_focus(*default_btn_ptr);
        Glib::signal_idle().connect_once(
            [default_btn_ptr]() { default_btn_ptr->grab_focus(); });
    }
}

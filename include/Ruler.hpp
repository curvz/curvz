...
public:
  ...
  sigc::signal<void()> &signal_guide_drag_cancel() {
    return m_sig_guide_cancel;
  }
...
private:
  ...
  std::pair<double, double> drag_screen_to_user(double offset_x,
  ...
  Gtk::Popover m_pop;
  CurvzSpinButton m_x_spin;
  CurvzSpinButton m_y_spin;
  // s290: ephemeral CanvasModel synced from m_state at popover-open time so
  // the spinners convert against the live document's intent + display unit.
  CanvasModel m_pop_model;

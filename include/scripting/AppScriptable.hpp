...
public:
    // s288 m2 — Pen-path performance callback. The enact_pen_path verb
    // routes through this; MainWindow wires it to a lambda that calls
    // Canvas::perform_pen_path (renamed s291 m2 from welcome_enact_pen_path).
    // May be empty; the verb is a silent no-op in that case (gift-shape
    // graceful degradation — the audience sees nothing rather than an error).
    using EnactPenPath = std::function<void(const std::string& d_string,
                                            double speed)>;

    // s288 m3 — SVG-file performance callback. The animate_svg verb
    // routes through this; MainWindow wires it to a lambda that calls
    // Canvas::perform_svg_file (renamed s291 m2 from welcome_animate_svg_file).
    // Same shape as EnactPenPath — may be empty (silent no-op).

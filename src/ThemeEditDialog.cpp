...
void ThemeEditDialog::show(Gtk::Window &parent, theme::Theme initial,
                           Motif initial_mode, CommittedFn on_committed) {
  m_working = initial;
...
void ThemeEditDialog::build_identity_section(Gtk::Box &root) {
  ...
  m_name_entry->on_commit([this]() {
    ...
    if (typed == m_working.header.name)
      return;
...
void ThemeEditDialog::build_body_section(Gtk::Box &root) {
  ...
  // ── Mode toggle row ─────────────────────────────────────────────
  auto *mode_row = Gtk::make_managed<Gtk::Box>(Gtk::Orientation::HORIZONTAL, 0);
  mode_row->set_halign(Gtk::Align::CENTER);
  mode_row->add_css_class("linked");
  body->append(*mode_row);

  m_mode_light_btn =
      Gtk::make_managed<curvz::widgets::ToggleButton>("dlg_te_mode_l", "Light");
  ...
  m_mode_dark_btn =
      Gtk::make_managed<curvz::widgets::ToggleButton>("dlg_te_mode_d", "Dark");
  curvz::utils::set_name(m_mode_dark_btn, "dlg_te_mode_d",
                         "theme_editor_dialog_mode_dark_btn");
  m_mode_dark_btn->set_tooltip_text("Preview the theme's dark-mode appearance");
  ...
  m_mode_light_btn->signal_toggled().connect([this]() {
    if (m_syncing)
      return;
    if (!m_mode_light_btn->get_active())
      return; // ignore the off-edge
    if (m_preview_mode == Motif::Light)
      return;
  ...
  m_mode_dark_btn->signal_toggled().connect([this]() {
    if (m_syncing)
      return;
    if (!m_mode_dark_btn->get_active())
      return;
    if (m_preview_mode == Motif::Dark)
      return;
  ...
  m_thumbnail->set_draw_func([this](const Cairo::RefPtr<Cairo::Context> &cr,
                                    int w,
                                    int h) { draw_thumbnail(cr, w, h); });
...
void ThemeEditDialog::build_button_row(Gtk::Box &root) {
  ...
  m_btn_cancel =
      Gtk::make_managed<curvz::widgets::Button>("dlg_te_cnc", "Cancel");
  ...
  m_btn_reset =
      Gtk::make_managed<curvz::widgets::Button>("dlg_te_rst", "Reset");
  ...
  curvz::utils::set_name(m_btn_ok, "dlg_te_ok", "theme_editor_dialog_ok_btn");
...
void ThemeEditDialog::refresh_mode_toggle_highlight() {
  if (!m_mode_dark_btn || !m_mode_light_btn)
    return;
...
void ThemeEditDialog::refresh_motif_swatches() {
  if (m_swatch_motif_artboard)
    m_swatch_motif_artboard->queue_draw();
  if (m_swatch_motif_workspace)
    m_swatch_motif_workspace->queue_draw();
  if (m_swatch_motif_creation)
    m_swatch_motif_creation->queue_draw();
}
...
Gtk::DrawingArea *make_swatch(int w, int h, ReadRGB read_rgb,
                              OnClick on_click) {
...
void ThemeEditDialog::build_tab_colors_snap(Gtk::Box &page) {
  ...
  auto motif_swatch_row = [this, &page](const char *label,
                                        Gtk::DrawingArea *&slot_ptr, auto get_r,
                                        auto get_g, auto get_b, auto set_r,
                                        auto set_g, auto set_b) {
    ...
    slot_ptr = make_swatch(
        ...
        [this, slot_ptr_addr = &slot_ptr, get_r, get_g, get_b, set_r, set_g,
         set_b]() {
          if (m_syncing)
            return;
          color::Color initial(get_r(m_working.motif, m_preview_mode),
                               get_g(m_working.motif, m_preview_mode),
                               get_b(m_working.motif, m_preview_mode), 1.0);
          ColorPickerPopover::shared().open(
              **slot_ptr_addr, initial, /*with_alpha=*/false,
              [this, slot_ptr_addr, set_r, set_g,
               set_b](const color::Color &c) {
                set_r(m_working.motif, m_preview_mode, c.r);
                set_g(m_working.motif, m_preview_mode, c.g);
                set_b(m_working.motif, m_preview_mode, c.b);
                if (*slot_ptr_addr)
                  (*slot_ptr_addr)->queue_draw();
                queue_thumbnail_redraw();
              });
        });
    row->append(*slot_ptr);
  ...
  auto art_set_r = [](theme::MotifSettings &m, Motif mo, double v) {
    if (mo == Motif::Light)
      m.light_artboard_r = v;
    else
      m.dark_artboard_r = v;
  };
  auto art_set_g = [](theme::MotifSettings &m, Motif mo, double v) {
    if (mo == Motif::Light)
      m.light_artboard_g = v;
    else
      m.dark_artboard_g = v;
  };
  auto art_set_b = [](theme::MotifSettings &m, Motif mo, double v) {
    if (mo == Motif::Light)
      m.light_artboard_b = v;
    else
      m.dark_artboard_b = v;
  };
  auto ws_set_r = [](theme::MotifSettings &m, Motif mo, double v) {
    if (mo == Motif::Light)
      m.light_workspace_r = v;
    else
      m.dark_workspace_r = v;
  };
  auto ws_set_g = [](theme::MotifSettings &m, Motif mo, double v) {
    if (mo == Motif::Light)
      m.light_workspace_g = v;
    else
      m.dark_workspace_g = v;
  };
  auto ws_set_b = [](theme::MotifSettings &m, Motif mo, double v) {
    if (mo == Motif::Light)
      m.light_workspace_b = v;
    else
      m.dark_workspace_b = v;
  };
  auto cr_set_r = [](theme::MotifSettings &m, Motif mo, double v) {
    if (mo == Motif::Light)
      m.light_creation_r = v;
    else
      m.dark_creation_r = v;
  };
  auto cr_set_g = [](theme::MotifSettings &m, Motif mo, double v) {
    if (mo == Motif::Light)
      m.light_creation_g = v;
    else
      m.dark_creation_g = v;
  };
  auto cr_set_b = [](theme::MotifSettings &m, Motif mo, double v) {
    if (mo == Motif::Light)
      m.light_creation_b = v;
    else
      m.dark_creation_b = v;
  };

  motif_swatch_row("Artboard", m_swatch_motif_artboard, art_r, art_g, art_b,
                   art_set_r, art_set_g, art_set_b);
  motif_swatch_row("Workspace", m_swatch_motif_workspace, ws_r, ws_g, ws_b,
                   ws_set_r, ws_set_g, ws_set_b);
  motif_swatch_row("Creation", m_swatch_motif_creation, cr_r, cr_g, cr_b,
                   cr_set_r, cr_set_g, cr_set_b);
  ...
  page.append(*make_subhead("Guides"));
  {
    ...
    m_swatch_guides = make_swatch(
        ...
        [this]() {
          ...
          ColorPickerPopover::shared().open(*m_swatch_guides, initial,
                                            /*with_alpha=*/false,
  ...
  page.append(*make_subhead("Units"));
  {
    ...
    auto sl =
        Gtk::StringList::create({"Pixels", "Inches", "Millimeters", "Points"});
    m_units_dd = Gtk::make_managed<curvz::widgets::DropDown>("dlg_te_un", sl);
    ...
    m_units_dd->set_selected(static_cast<guint>(m_working.units.display_unit));
  ...
  page.append(*make_subhead("Snap"));
  {
    ...
    auto add_chk = [&](curvz::widgets::CheckButton *&slot,
                       std::string_view abbrev, const char *label, bool initial,
                       auto setter, int col, int row) {
      slot = Gtk::make_managed<curvz::widgets::CheckButton>(abbrev, label);
      slot->set_active(initial);
      slot->signal_toggled().connect([this, slot, setter]() {
        if (m_syncing)
          return;
...
void ThemeEditDialog::build_tab_grid(Gtk::Box &page) {
  ...
  // can be negative (Distance).
  {
    ...
    auto add_xy_row = [this](Gtk::Box &container, const char *label,
                             SpinType type, double initial_x, double initial_y,
                             CurvzSpinButton *&slot_x, CurvzSpinButton *&slot_y,
      ...
      row->append(*slot_y);
      if (auto *ul = slot_y->get_unit_label())
        row->append(*ul);
    ...
    add_xy_row(
        page, "Spacing", SpinType::Width, m_working.grid.spacing_x,
        m_working.grid.spacing_y, m_grid_spacing_x, m_grid_spacing_y,
        [](auto &gd, double v) { gd.spacing_x = v; },
        [](auto &gd, double v) { gd.spacing_y = v; });
    add_xy_row(
        page, "Offset", SpinType::Distance, m_working.grid.offset_x,
        m_working.grid.offset_y, m_grid_offset_x, m_grid_offset_y,
  ...
  // Color + alpha.
  {
    ...
    m_swatch_grid = make_swatch(
        ...
        [this]() {
          if (m_syncing)
            return;
          color::Color initial(m_working.grid.color_r, m_working.grid.color_g,
                               m_working.grid.color_b, 1.0);
          ColorPickerPopover::shared().open(*m_swatch_grid, initial,
                                            /*with_alpha=*/false,
                                            [this](const color::Color &c) {
    ...
    row->append(*lbl_a);
    auto adj = Gtk::Adjustment::create(m_working.grid.color_a * 100.0, 0.0,
                                       100.0, 1.0, 10.0);
...
void ThemeEditDialog::build_tab_margins(Gtk::Box &page) {
  ...
  // CurvzSpinButton (integers, no unit).
  ...
  auto add_dim_pair_row = [this](Gtk::Box &container, const char *label,
                                 SpinType type, double init_a, double init_b,
                                 CurvzSpinButton *&slot_a,
                                 CurvzSpinButton *&slot_b, auto setter_a,
                                 auto setter_b, const char *sublabel_a,
    ...
    row->append(*slot_a);
    if (auto *ul = slot_a->get_unit_label())
      row->append(*ul);
    ...
    row->append(*slot_b);
    if (auto *ul = slot_b->get_unit_label())
      row->append(*ul);
  ...
  auto add_dim_row = [this](Gtk::Box &container, const char *label,
    ...
    row->append(*slot);
    if (auto *ul = slot->get_unit_label())
      row->append(*ul);
    container.append(*row);
  };
  ...
  auto add_count_row = [this](Gtk::Box &container, std::string_view abbrev,
                              const char *label, int initial,
                              curvz::widgets::SpinButton *&slot, auto setter) {
    auto *row = Gtk::make_managed<Gtk::Box>(Gtk::Orientation::HORIZONTAL, 8);
    row->set_margin_start(8);
    auto *lbl = Gtk::make_managed<Gtk::Label>(label);
    lbl->set_xalign(0.0f);
    ...
    slot = Gtk::make_managed<curvz::widgets::SpinButton>(abbrev, adj, 1.0, 0);
    slot->set_hexpand(true);
  ...
  add_dim_pair_row(
      page, "Vertical", SpinType::Distance, m_working.margins.top,
      m_working.margins.bottom, m_margin_top, m_margin_bottom,
      [](auto &m, double v) { m.top = v; },
      [](auto &m, double v) { m.bottom = v; }, "T", "B");
  add_dim_pair_row(
      page, "Horizontal", SpinType::Distance, m_working.margins.left,
      m_working.margins.right, m_margin_left, m_margin_right,
      [](auto &m, double v) { m.left = v; },
      [](auto &m, double v) { m.right = v; }, "L", "R");

  // Columns.
  page.append(*make_subhead("Columns"));
  add_count_row(page, "dlg_te_mg_col", "Count", m_working.margins.columns,
                m_margin_cols, [](auto &m, int v) { m.columns = v; });
  ...
  add_dim_row(page, "Gap", SpinType::Width, m_working.margins.col_gap,
              m_margin_col_gap, [](auto &m, double v) { m.col_gap = v; });

  // Rows.
  page.append(*make_subhead("Rows"));
  add_count_row(page, "dlg_te_mg_row", "Count", m_working.margins.rows,
                m_margin_rows, [](auto &m, int v) { m.rows = v; });
  ...
  add_dim_row(page, "Gap", SpinType::Width, m_working.margins.row_gap,
              m_margin_row_gap, [](auto &m, double v) { m.row_gap = v; });
  ...
  page.append(*make_subhead("Color"));
  {
    ...
    m_swatch_margin = make_swatch(
        ...
        [this]() {
          ...
          ColorPickerPopover::shared().open(*m_swatch_margin, initial,
                                            /*with_alpha=*/false,
                                            [this](const color::Color &c) {
    ...
    auto *lbl_a = Gtk::make_managed<Gtk::Label>("Alpha %");
    row->append(*lbl_a);
    auto adj = Gtk::Adjustment::create(m_working.margins.color_a * 100.0, 0.0,
                                       100.0, 1.0, 10.0);
...
void ThemeEditDialog::sync_from_working() {
  ...
  if (m_margin_cols) {
    m_margin_cols->set_value(static_cast<double>(m_working.margins.columns));
  }
  if (m_margin_col_gap) {
  ...
  if (m_margin_rows) {
    m_margin_rows->set_value(static_cast<double>(m_working.margins.rows));
  }
  if (m_margin_row_gap) {

...
std::string sanitise_for_xml_id(const std::string &name) {
  ...
  while (i < name.size()) {
    ...
    if (i + 1 < name.size() && (unsigned char)name[i] == 0xC2 &&
  ...
  if (out.empty() || std::isdigit((unsigned char)out[0]) || out[0] == '-' ||
      out[0] == '.') {
...
// ── encode_svg_id ───────────────────────────────────────────────────
std::string encode_svg_id(const std::string &name, const std::string &iid) {
  const bool name_empty = name.empty();
  const bool iid_empty = iid.empty();

  if (name_empty && iid_empty)
    return "";
...
int doc_anchor_count(const Curvz::CurvzDocument &doc) {
  ...
  for (const auto &l : doc.layers) {
    ...
    if (l->is_grid_layer())
      continue;
    if (l->is_margin_layer())
      continue;
    if (l->is_ref_layer())
      continue;
    if (l->is_measure_layer())
      continue;
...
Curvz::SceneNode *find_object_parent(Curvz::CurvzDocument *doc,
  ...
  for (auto &layer : doc->layers) {
    for (auto &child : layer->children) {
      if (child.get() == target)
        return nullptr; // top-level
      if (auto *r = walk(child.get()))
        return r;
    }
  }
  return nullptr;
...
void box_blur_argb32(unsigned char *data, int stride, int width, int height,
                     int radius) {
  if (!data || width <= 0 || height <= 0 || radius <= 0)
    return;
    ...
    for (int x = 0; x < width; ++x) {
      ...
      for (int y = 0; y < height; ++y) {
        ...
        const unsigned char *pin =
            scratch.data() + (size_t)y_in * stride + x * 4;
        const unsigned char *pout =
            scratch.data() + (size_t)y_out * stride + x * 4;
        sb += pin[0];
        sb -= pout[0];
        sg += pin[1];
        sg -= pout[1];
...
build_gradient_pattern(const Curvz::FillStyle &fill, double bbox_x,
...
void cairo_set_source_pixbuf(const Cairo::RefPtr<Cairo::Context> &cr,
                             const Glib::RefPtr<Gdk::Pixbuf> &pb, double x,
                             double y) {
  if (!cr || !pb)
    return;
  int w = pb->get_width();
  ...
  auto surf =
      Cairo::ImageSurface::create(Cairo::ImageSurface::Format::ARGB32, w, h);
  surf->flush();
...
void render_drop_shadow_under(const Cairo::RefPtr<Cairo::Context> &cr,
  ...
  const double pad_doc =
      std::ceil(blur_doc * 2.0) + std::abs(dx_doc) + std::abs(dy_doc) + 4.0;
  ...
  const double ctm_scale = std::sqrt(ctm.xx * ctm.xx + ctm.yx * ctm.yx);

  // Offscreen ARGB32 (premultiplied) — only format box_blur supports.
  auto surf =
      Cairo::ImageSurface::create(Cairo::Surface::Format::ARGB32, pix_w, pix_h);
  if (!surf)
    return;
  ...
  if (radius_px > 0) {
    box_blur_argb32(surf->get_data(), surf->get_stride(), pix_w, pix_h,
                    radius_px);
    surf->mark_dirty();
  }
  ...
  const double final_a = std::max(0.0, std::min(1.0, color_a * opacity));
  ...
  return std::visit(
      [grid, row](const auto &s) -> FieldWidget {
        ...
        if constexpr (std::is_same_v<T, curvz::utils::TextField>) {
          ...
          auto adj = Gtk::Adjustment::create(s.default_value, s.min, s.max,
          ...
          if (s.default_index >= 0 && s.default_index < (int)s.items.size())
  ...
  return std::visit(
      [](const auto &w) -> curvz::utils::FormFieldValue {
        ...
        if constexpr (std::is_same_v<T, Gtk::Entry *>) {
          ...
          int idx = w.dropdown ? (int)w.dropdown->get_selected() : 0;
          v.value = idx;
...
// cleans itself up.
void build_dialog(
    Gtk::Window &parent, const std::string &title, const std::string &detail,
    const std::vector<curvz::utils::FormField> &fields,
    const std::vector<std::string> &buttons, int default_button,
    int cancel_button,
    std::function<
        void(int, const std::map<std::string, curvz::utils::FormFieldValue> &)>
        on_finished) {
  ...
  for (size_t i = 0; i < buttons.size(); ++i) {
    ...
    curvz::utils::set_name(btn, "dlg_alert_btn", "curvz_alert_dialog_button");
    const int idx = (int)i;
  ...
  key_ctrl->signal_key_pressed().connect(
      [dlg, chosen, fired, cancel_button](guint keyval, guint,
  ...
  dlg->signal_close_request().connect(
      [dlg, chosen, fired, cancel_button, on_finished, collect_values,
       cb_fired]() {
        if (*cb_fired)
          return false; // already handled — let close proceed
        *cb_fired = true;
        LOG_INFO("build_dialog: close_request fired={} chosen={}", *fired,
                 *chosen);
        if (!*fired) {
...
void show_alert(Gtk::Window &parent, const std::string &title,
...
void show_confirm(Gtk::Window &parent, const std::string &title,
                  const std::string &detail,
                  const std::vector<std::string> &buttons, int default_button,
                  int cancel_button, std::function<void(int)> callback) {
  build_dialog(
      parent, title, detail, /*fields=*/{}, buttons, default_button,
      cancel_button,
      [callback](int idx, const std::map<std::string, FormFieldValue> &) {
        callback(idx);
      });
}
...
void show_form(
    ...
    const std::vector<std::string> &buttons, int default_button,
    int cancel_button,
    std::function<void(int, const std::map<std::string, FormFieldValue> &)>
        callback) {
  build_dialog(parent, title, detail, fields, buttons, default_button,
               cancel_button, std::move(callback));
}
...
Gtk::Widget *
make_path_override_row(const char *label_text, const std::string &current_value,
                       const std::string &default_path, const char *tooltip,
                       bool pick_folder, Gtk::Window *dialog_parent,
  ...
  browse->set_tooltip_text(pick_folder ? "Browse for a folder"
                                       : "Browse for a file");
  row->append(*browse);
  ...
  auto commit = [entry, on_commit]() { on_commit(entry->get_text()); };
  ...
  browse->signal_clicked().connect([dialog_parent, pick_folder, entry,
                                    on_commit, default_path]() {
    ...
    if (!seed.empty()) {
      try {
        ...
        if (!init.empty() && fs::exists(init)) {
          dialog->set_initial_folder(Gio::File::create_for_path(init.string()));
        }
      } catch (...) {
      }
    }

    auto handler = [dialog, entry, on_commit,
                    pick_folder](Glib::RefPtr<Gio::AsyncResult> &result) {
      try {
        auto file = pick_folder ? dialog->select_folder_finish(result)
...
PathData build_arc(double bx, double bw, double base_y, double amplitude,
                   int count) {
  if (count <= 2)
...
PathData build_wave(double bx, double bw, double base_y, double amplitude,
                    int count) {
  if (count <= 2)
    return build_flat(bx, bw, base_y, count);
  PathData pd;
...
bool requires_three_anchors(int preset_idx) {
  ...
  return preset_idx == ARC_UP || preset_idx == ARC_DOWN ||
         preset_idx == BULGE || preset_idx == SQUEEZE || preset_idx == WAVE;
}

} // namespace warp_presets

void generate_warp_preset(int preset_idx, double bx, double by, double bw,
                          double bh, int top_count, int bot_count,
...
std::string path_data_to_svg_d(const ::Curvz::PathData &pd) {
  ...
  std::ostringstream d;
  d << "M " << fmt2_path_d(pd.nodes[0].x) << " " << fmt2_path_d(pd.nodes[0].y);
  ...
  for (int i = 0; i < segs; ++i) {
    ...
    bool a_degen =
        (std::abs(a.cx2 - a.x) < 1e-6 && std::abs(a.cy2 - a.y) < 1e-6);
    bool b_degen =
        (std::abs(b.cx1 - b.x) < 1e-6 && std::abs(b.cy1 - b.y) < 1e-6);
    if (a_degen && b_degen) {
...
constexpr NamedColor k_named_colors[] = {
    ...
    {"purple", 0x800080},  {"grey", 0x808080},    {"gray", 0x808080},
...
std::string fill_style_to_fill_attr(const ::Curvz::FillStyle &fs) {
  ...
  case FS::Type::None:
    return "none";
  case FS::Type::CurrentColor:
    return "currentColor";
  case FS::Type::Solid: {
    char buf[8];
    std::snprintf(buf, sizeof(buf), "#%02x%02x%02x",
                  (int)std::round(fs.r * 255), (int)std::round(fs.g * 255),
  ...
  case FS::Type::RadialGradient: {
    ...
    char buf[8];
    std::snprintf(buf, sizeof(buf), "#%02x%02x%02x", (int)std::round(r * 255),

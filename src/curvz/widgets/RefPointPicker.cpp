// curvz/widgets/RefPointPicker.cpp ───────────────────────────────────────────
...
RefPointPicker::RefPointPicker(std::string_view name,
        ...
        });
    {
        ...
        click->signal_pressed().connect(
            [this](int /*n_press*/, double x, double y) {
                if (m_mode != Mode::Preset) return;
                Preset p;
                if (!pixel_to_preset(x, y, &p)) return;
                set_preset(p);
            });
    ...
    m_arbitrary_chk.signal_toggled().connect([this]() {
        if (m_loading) return;
        set_mode(m_arbitrary_chk.get_active() ? Mode::Arbitrary
                                              : Mode::Preset);
    });
    ...
    {
        auto* x_col = Gtk::make_managed<Gtk::Box>(
        ...
        auto* x_row = Gtk::make_managed<Gtk::Box>(
        ...
        m_sp_x->signal_internal_changed().connect([this](double v) {
            if (m_loading) return;
            if (m_mode != Mode::Arbitrary) return;  // preset mode ignores edits
            m_arb_x = v;
        ...
        if (m_x_unit_lbl) x_row->append(*m_x_unit_lbl);
        x_col->append(*x_row);
        append(*x_col);
    }
    ...
    // ── Y column ────────────────────────────────────────────────────────
    {
        auto* y_col = Gtk::make_managed<Gtk::Box>(
            Gtk::Orientation::VERTICAL, 1);
        m_y_lbl.set_text("Y");
        ...
        auto* y_row = Gtk::make_managed<Gtk::Box>(
            Gtk::Orientation::HORIZONTAL, 2);
        ...
        m_sp_y->signal_internal_changed().connect([this](double v) {
            if (m_loading) return;
            if (m_mode != Mode::Arbitrary) return;
            m_arb_y = v;
            m_sig_point_changed.emit(m_arb_x, m_arb_y);
        ...
        if (m_y_unit_lbl) y_row->append(*m_y_unit_lbl);
        y_col->append(*y_row);
        append(*y_col);
    }
...
void RefPointPicker::set_mode(Mode m) {
    ...
    emit("mode_changed",
         ScriptValue::text(m_mode == Mode::Arbitrary ? "arbitrary"
                                                    : "preset"));
    auto [px, py] = point();
    m_sig_point_changed.emit(px, py);
    m_grid_area.queue_draw();
...
void RefPointPicker::set_arbitrary_xy(double doc_x, double doc_y) {
    ...
    m_arb_y = doc_y;
    m_loading = true;
    if (m_sp_x) m_sp_x->set_internal_value(doc_x);
    if (m_sp_y) m_sp_y->set_internal_value(doc_y);
    m_loading = false;
    m_sig_point_changed.emit(m_arb_x, m_arb_y);
    emit("point_changed", ScriptValue::real(m_arb_x));
}
...
ScriptValue RefPointPicker::invoke_leaf(std::string_view verb,
    ...
    if (verb == "set_preset") {
        ...
        if (!preset_from_name(args[0].s, &p))
            throw std::runtime_error(
                "RefPointPicker.set_preset: unknown preset '" +
                args[0].s + "'");
        set_preset(p);
        return ScriptValue::null();
    }
    auto as_d = [](const ScriptValue& v) -> double {
        if (v.kind == ValueKind::Double) return v.d;
        if (v.kind == ValueKind::Int)    return static_cast<double>(v.i);
    ...
    if (verb == "set_bbox") {
        ...
        set_bbox(as_d(args[0]), as_d(args[1]),
                 as_d(args[2]), as_d(args[3]));
        return ScriptValue::null();
    }
    throw std::runtime_error(
        "RefPointPicker: unknown verb '" + std::string(verb) + "'");
}
...
ScriptValue RefPointPicker::query_leaf(std::string_view property) const {
    ...
    if (property == "x") { auto [px, py] = point(); (void)py; return ScriptValue::real(px); }
    if (property == "y") { auto [px, py] = point(); (void)px; return ScriptValue::real(py); }
    if (property == "bbox_x") return ScriptValue::real(m_bbox_x);
    if (property == "bbox_y") return ScriptValue::real(m_bbox_y);
    if (property == "bbox_w") return ScriptValue::real(m_bbox_w);
    if (property == "bbox_h") return ScriptValue::real(m_bbox_h);
    return ScriptValue::null();
}
...
double RefPointPicker::preset_doc_x(Preset p) const {
    ...
    switch (p) {
        case Preset::NW: case Preset::W: case Preset::SW: return left;
        case Preset::N:  case Preset::C: case Preset::S:  return midx;
        case Preset::NE: case Preset::E: case Preset::SE: return right;
    }
    return midx;
}
...
double RefPointPicker::preset_doc_y(Preset p) const {
    ...
    switch (p) {
        case Preset::NW: case Preset::N: case Preset::NE: return top;
        case Preset::W:  case Preset::C: case Preset::E:  return midy;
        case Preset::SW: case Preset::S: case Preset::SE: return bottom;
    }
    return midy;
}
...
bool RefPointPicker::pixel_to_preset(double px, double py,
    ...
    if (inner_w <= 0 || inner_h <= 0) return false;
    double fx = (px - margin) / inner_w;
    double fy = (py - margin) / inner_h;
    if (fx < -0.1 || fx > 1.1 || fy < -0.1 || fy > 1.1) return false;
...
void RefPointPicker::refresh_xy_display() {
    auto [px, py] = point();
    m_loading = true;
    if (m_sp_x) m_sp_x->set_internal_value(px);
    if (m_sp_y) m_sp_y->set_internal_value(py);
    m_loading = false;
}
...
void RefPointPicker::apply_mode_appearance() {
    ...
    if (m_sp_x) m_sp_x->set_editable(arb);
    if (m_sp_y) m_sp_y->set_editable(arb);
}
...
void RefPointPicker::on_grid_draw(const Cairo::RefPtr<Cairo::Context>& cr,
    ...
    const double xs[3] = { margin,
                           margin + iw * 0.5,
                           margin + iw };
    const double ys[3] = { margin,
                           margin + ih * 0.5,
                           margin + ih };
    static const Preset table[3][3] = {
        { Preset::NW, Preset::N, Preset::NE },
    ...
    for (int r = 0; r < 3; ++r) {
        for (int c = 0; c < 3; ++c) {
            ...
            const bool selected = (m_mode == Mode::Preset &&
                                   p == m_last_preset);

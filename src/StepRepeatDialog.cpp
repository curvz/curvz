...
StepRepeatDialog::StepRepeatDialog()
    : m_copies(Gtk::Adjustment::create(3, 1, 999, 1, 10), 0.0, 0),
      m_rotate_enable("Rotate around pivot")
{
    curvz::utils::set_name(*this, "dlg_sr", "step_repeat_dialog_root");
    set_title("Step and Repeat");
    set_resizable(false);
    ...
    m_btn_cancel.signal_clicked().connect(
        sigc::mem_fun(*this, &StepRepeatDialog::on_cancel));
    ...
    m_btn_ok.signal_clicked().connect(
        sigc::mem_fun(*this, &StepRepeatDialog::on_ok));
    btn_bar->append(m_btn_ok);

    m_outer.append(*btn_bar);
    set_child(m_outer);

    // WM close (X button / ESC via hide-on-close) → treat as cancel so the
    // crosshair is hidden. set_hide_on_close routes close-request to hide(),
    // which emits signal_hide; hook that instead of close-request.
...
// ── show ──────────────────────────────────────────────────────────────────────
void StepRepeatDialog::show(Gtk::Window&      parent,
                            ...
                            ApplyCb            apply_cb)
{
    ...
    // Dialog is non-modal: canvas must receive clicks while the dialog is
    // open so the user can drag the SnR refpt on canvas.  transient_for
    // still anchors the dialog above its parent window.
    set_modal(false);
    present();
...
void StepRepeatDialog::refresh_angle_ui() {
...
void StepRepeatDialog::emit_pivot_change(bool visible) {
...
void StepRepeatDialog::set_pivot_from_canvas(double px, double py) {
    m_pivot_dx = px;
    m_pivot_dy = py;
...
void StepRepeatDialog::on_preview_draw(
...
void StepRepeatDialog::doc_to_preview_px(
...
void StepRepeatDialog::preview_px_to_doc(
...
// ── Preview interaction ───────────────────────────────────────────────────────
void StepRepeatDialog::on_preview_press(int, double x, double y) {
...
void StepRepeatDialog::on_preview_drag_update(
...
void StepRepeatDialog::on_ok() {
    ...
    // hide() will emit signal_hide → crosshair off.
    hide();
    if (m_apply_cb) m_apply_cb(r);
}

void StepRepeatDialog::on_cancel() {
    hide();
}

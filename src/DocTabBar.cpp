...
static bool tab_icontains(const std::string& haystack, const std::string& needle) {
...
DocTabBar::DocTabBar() : Gtk::Box(Gtk::Orientation::HORIZONTAL) {
    ...
    m_scroll_left.set_size_request(22, -1);
    ...
    m_scroll_left.signal_clicked().connect([this]() {
        ...
        if (adj) scroll_by(-adj->get_page_size() * 0.5);
    });
    ...
    curvz::utils::set_name(m_scroll_right, "dt_r", "doc_tab_bar_scroll_right_btn");
    ...
    scroll_ctx->signal_pressed().connect([this](int, double x, double y) {
        show_bar_menu(x, y);
    });
...
void DocTabBar::show_tab_menu(int idx, double x, double y) {
    ...
    if (can_remove) {
        auto item_rm = Gio::MenuItem::create("Remove This Document", "tabctx.remove");
    ...
    act_new->signal_activate().connect([this](const Glib::VariantBase&) {
        m_sig_add.emit();
    });
    ...
    if (can_remove) {
        ...
        act_rm->signal_activate().connect([this, capture_idx](const Glib::VariantBase&) {
...
void DocTabBar::rebuild() {
    ...
    for (int i = 0; i < (int)m_project->documents.size(); ++i) {
        ...
        for (auto ext : {".svg", ".curvz"}) {
            std::string e{ext};
            if (name.size() > e.size() &&
                name.substr(name.size() - e.size()) == e)
        ...
        if (i == active) tab->add_css_class("doc-tab-active");
        ...
        const int idx = i;
        gesture->signal_pressed().connect([this, idx](int, double, double) {
            m_sig_activated.emit(idx);
        });
        ...
        gesture_ctx->signal_pressed().connect([this, idx, tab](int, double x, double y) {
    ...
    LOG_DEBUG("DocTabBar rebuilt: {} tabs, active={}", m_project->documents.size(), active);

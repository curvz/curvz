// AnimatingSvgParser.cpp ────────────────────────────────────────────────────
//
// s290 m1a — Peer class to SvgParser. This file is a copy of
// SvgParser.cpp with `parse_svg` and `parse_svg_file` lifted into the
// AnimatingSvgParser class as `parse` and `parse_file` respectively.
// The static helpers are duplicated by translation-unit-local linkage
// (each `.cpp` has its own statics; no collision with the base parser).
//
// s290 m1b.1 — `on_path` fan-out wired at the `push_into_parent`
// funnel inside parse() AND at a sibling `commit_child` funnel for
// container-internal commits (Compound/Blend/Warp/ClipGroup children
// that bypass push_into_parent). Both funnels share `emit_if_path`
// so there's exactly ONE emit point.
//
// s290 m1b.2 — adds container bracketing. Compounds and Groups fire
// `on_compound_begin/end` and `on_group_begin/end` around their
// contained path events. Two construction patterns share the same
// emit shape:
//   - Stack-based <g>: begin fires when the container is committed
//     to its parent; end fires at the matching </g> stack-pop.
//   - Atomic <path data-curvz-compound> / multi-subpath: begin fires
//     before the child-commit loop, end fires after.
// Nesting is preserved naturally — the begin/end pairs interleave
// with the on_path events in document order.
//
// s290 m1b.3 — adds setup events.
//   - on_doc_metadata fires once at the end of the <svg> root tag
//     handler, with doc->canvas fully populated.
//   - on_layer_begin fires after current_layer is assigned in each
//     of the six layer-creation sites. Layer-type filtering: only
//     Type::Layer fires events; the technical layer kinds (RefLayer,
//     MeasureLayer, GuideLayer, Grid, Margin) are infrastructure
//     and stay silent.
//   - on_layer_end fires from the existing stack-pop seam, extended
//     to handle Type::Layer. The post-parse safety-net layer (when
//     doc->layers is empty) fires begin and end adjacently.
//
// The exported `parse_path_d_bridge` from the base parser is NOT
// duplicated here — the base parser keeps owning that public surface;
// nothing in this file needs to call it externally.
//
// See AnimatingSvgParser.hpp for the architectural rationale (why peer
// class instead of subclass; what m1b.2 / m1b.3 will add).

#include "AnimatingSvgParser.hpp"
...
// parse_path_d_bridge: NOT duplicated in this file. The base
// SvgParser.cpp keeps owning that exported symbol; nothing in this
// translation unit needs to call it externally (the local static
// parse_path_d covers in-file use).
...
// ── AnimatingSvgParser::parse ─────────────────────────────────────────────

std::unique_ptr<CurvzDocument> AnimatingSvgParser::parse(const std::string& svg) {
    ...
    // s290 m1b.1 — pull the emitter pointer into a local so the
    // capture-by-reference lambda below doesn't need implicit-this
    // semantics. Same pointer the class member holds; nullable.
    SvgEmitter* emitter = m_emitter;
    ...
    // s290 m1b.1 — single emit funnel. push_into_parent below
    // routes top-level node commits; commit_child further down
    // routes container-child commits (Compound/Blend/Warp/ClipGroup
    // children that go directly into a parent's children vector
    // without traversing push_into_parent). Both call emit_if_path
    // so there's exactly ONE place where on_path actually fires.
    //
    // The iid-pump principle: find the seam, build the small
    // helper at it, never scatter the emit call. Two funnels exist
    // here (push_into_parent and commit_child) because the parser
    // has two materially-different commit shapes — top-of-tree
    // commits go through stack/clipdefs routing, container-internal
    // commits don't. Both shapes share emit semantics; the helper
    // captures that sharing.
    auto emit_if_path = [&](const SceneNode* node) {
        if (emitter && node && node->type == SceneNode::Type::Path) {
            emitter->on_path(*node);
        }
    };

    auto push_into_parent = [&](std::unique_ptr<SceneNode> node) {
        emit_if_path(node.get());
    ...
    // s290 m1b.1 — second funnel for container-child commits.
    // Used at every site that today does
    // `<parent>->children.push_back(std::move(<child>))` directly
    // (Compound children built from data-curvz-compound; Compound
    // children built from multi-subpath flatten; ClipGroup / Blend
    // / Warp / Group container nodes; multi-layer top-level
    // children). Same emit semantics as push_into_parent's path
    // gate; different routing target (the parent passed in).
    auto commit_child = [&](SceneNode* parent,
                            std::unique_ptr<SceneNode> child) {
        emit_if_path(child.get());
    ...
    for (const auto& tag : tags) {
        ...
        if (tag[0] == '/') {
            ...
            if ((tag == "/g" || tag.rfind("/g", 0) == 0) && !stack.empty()) {
                ...
                // s290 m1b.2 — bracketing end event for stack-based
                // containers. Fires just before the stack pop so the
                // event order matches the document structure.
                // s290 m1b.3 — extended to fire on_layer_end for
                // Type::Layer (technical layer kinds still skip).
                if (emitter) {
                    if (entry.node->type == SceneNode::Type::Compound)
                        emitter->on_compound_end();
                    else if (entry.node->type == SceneNode::Type::Group)
                        emitter->on_group_end();
                    else if (entry.node->type == SceneNode::Type::Layer)
                        emitter->on_layer_end();
                }
                stack.pop_back();
        ...
        if (tag.rfind("svg ", 0) == 0 || tag == "svg") {
            ...
            // s290 m1b.3 — fire on_doc_metadata once the svg-root
            // handler has finished populating the canvas model. At
            // this point doc->canvas reflects everything the parser
            // knows about page geometry: viewBox, intended size and
            // units, physical/pixel mode, ratio/quality. Subsequent
            // tag handlers add child content; they do not touch the
            // canvas model.
            if (emitter) emitter->on_doc_metadata(doc->canvas);
...
bool is_guide_layer   = (attr(tag, "data-curvz-guide-layer") == "1");
            ...
            if (is_clipgroup && !stack.empty()) {
                ...
                commit_child(stack.back().node, std::move(cg));
            ...
            if (is_blend && !stack.empty()) {
                ...
                commit_child(stack.back().node, std::move(bl));
            ...
            if (is_warp && !stack.empty()) {
                ...
                commit_child(stack.back().node, std::move(wp));
            ...
            if ((is_group || is_compound) && !stack.empty()) {
                ...
                // on_group_begin declares the container to the
                // consumer; subsequent on_path events apply to it
                // until the matching end fires at stack-pop time.
                if (emitter && gptr) {
                    if (gptr->type == SceneNode::Type::Compound)
                        emitter->on_compound_begin(*gptr);
                    else if (gptr->type == SceneNode::Type::Group)
                        emitter->on_group_begin(*gptr);
                }
                ...
                // s290 m1b.3 — on_layer_begin for Type::Layer. current_layer
                // points at the just-pushed node; metadata (name, opacity,
                // visibility, transform) is complete. End fires from the
                // stack-pop seam when the matching </g> closes.
                if (emitter) emitter->on_layer_begin(*current_layer);
            }
            continue;
        }
        ...
        if (!current_parent() && in_defs == 0) {
            ...
            stack.push_back({current_layer, AffineMatrix{}});
            doc->layers.push_back(std::move(l));
            // s290 m1b.3 — on_layer_begin for the bare-objects
            // fallback layer. End fires from stack-pop when
            // parsing reaches the closing tag that pops past this
            // implicit layer.
            if (emitter) emitter->on_layer_begin(*current_layer);
        }
        ...
        if (tag.rfind("line ", 0) == 0 && attr(tag, "data-curvz-guide") == "1") {
            if (current_layer && current_layer->is_guide_layer()) {
                ...
                commit_child(current_layer, std::move(g));
            }
            continue;
        }
        ...
        if (tag.rfind("line ", 0) == 0 && attr(tag, "data-curvz-measure") == "1") {
            if (current_layer && current_layer->is_measure_layer()) {
                ...
                commit_child(current_layer, std::move(mn));
            }
            continue;
        }
            ...
            if (!d_val.empty()) {
                ...
                if (is_compound_path) {
                    ...
                    for (size_t ci = 0; ci < sub_ds.size(); ++ci) {
                        ...
                        commit_child(compound.get(), std::move(child));
                    }

                    // s290 m1b.2 — atomic compound: fire on_compound_end
                    // after all children are committed, before the
                    // compound itself goes into its parent via
                    // push_into_parent below.
                    if (emitter) emitter->on_compound_end();
                ...
                if (subpaths.size() > 1) {
                    ...
                    // atomic compound above: build children in a loop,
                    // commit each, then push the compound itself.
                    // Bracketing fires around the loop.
                    if (emitter) emitter->on_compound_begin(*compound);
    ...
    if (doc->layers.empty()) {
        ...
        l->type = SceneNode::Type::Layer;
        l->name = "Layer 1";
        // s290 m1b.3 — safety-net layer: emit begin and end as a
        // pair right here. The layer doesn't go on the stack and
        // has no contents, so the standard stack-pop seam won't
        // fire on_layer_end for it. We fire both adjacently so the
        // begin/end count stays balanced for the empty-doc case.
        SceneNode* lptr = l.get();
        if (emitter) emitter->on_layer_begin(*lptr);
        doc->layers.push_back(std::move(l));
        if (emitter) emitter->on_layer_end();
    }
...
std::unique_ptr<CurvzDocument> AnimatingSvgParser::parse_file(const std::string& path) {
    std::ifstream f(path);
    if (!f) {
        LOG_ERROR("AnimatingSvgParser: cannot open '{}'", path);
        return nullptr;
    }
    std::ostringstream ss;
    ss << f.rdbuf();
    // s203 m4 — Outer catastrophic guard. The per-element try/catch inside
    // parse() catches anything thrown from a single tag's handler. This
    ...
    // per-element try block comment in parse() for why.
    std::unique_ptr<CurvzDocument> doc;
    try {
        doc = parse(ss.str());
    } catch (const std::exception& e) {
        LOG_ERROR("AnimatingSvgParser: catastrophic parse failure on '{}' — std::exception "
                  "what='{}'", path, e.what());
        std::fprintf(stderr,
                     "AnimatingSvgParser: catastrophic parse failure on '%s' — %s\n",
                     path.c_str(), e.what());
        return nullptr;
    } catch (...) {
        LOG_ERROR("AnimatingSvgParser: catastrophic parse failure on '{}' — unknown "
                  "exception", path);
        std::fprintf(stderr,
                     "AnimatingSvgParser: catastrophic parse failure on '%s' — unknown "
                     "exception type\n", path.c_str());
        return nullptr;
    }

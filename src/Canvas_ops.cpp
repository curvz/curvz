...
// ── Canvas::set_refpt_as_export_origin / clear_refpt_export_origin (s176 m1) ─
//
// Refpt-as-export-origin is the doc-wide flag that picks the (0, 0) anchor
// for refpt coordinate exports. Document-wide invariant: AT MOST ONE Ref
// in the doc has the flag set. Setter enforces this by walking every Ref
// in the document and clearing the flag before setting it on the target.
//
// Both walks are shallow — refpts live as direct children of RefLayer in
// Curvz's data model. We don't recurse into Path children or other
// non-RefLayer nodes; a "Ref" inside a Path container would be a structural
// anomaly the parser wouldn't produce.
//
// Non-undoable in m1. The flag is metadata about role; not destructive
// data. Banked as backlog: wrap as an EditFlag command using the s170
// m1+m2 iid-based capture pattern.

namespace {
// Walk every visible RefLayer in the document and apply f(ref) to every
// Ref child. Local helper — keeps the two methods below symmetric.
template <typename F>
void for_each_ref(CurvzDocument *doc, F &&f) {
  if (!doc) return;
  for (auto &layer : doc->layers) {
    if (!layer || !layer->is_ref_layer()) continue;
    for (auto &child : layer->children) {
      if (!child || !child->is_ref()) continue;
      f(child.get());
    }
  }
}
...
void Canvas::set_refpt_as_export_origin(SceneNode *ref) {
  if (!m_doc || !ref || !ref->is_ref()) {
    LOG_WARN("Canvas: set_refpt_as_export_origin ignored — doc={} ref={} "
             "is_ref={}",
             (void *)m_doc, (void *)ref, ref ? ref->is_ref() : false);
    return;
  }

  // Enforce at-most-one. Clear every Ref's flag, then set the target's.
  // This handles the case where the target was already the origin
  // (idempotent — clear then set still ends in the right state) and
  // the case where another Ref was the origin (cleared as a side
  // effect, which is the desired invariant-restoring behaviour).
  int cleared = 0;
  for_each_ref(m_doc, [&cleared](SceneNode *r) {
    if (r->is_export_origin) {
      r->is_export_origin = false;
      ++cleared;
    }
  });
  ...
  m_sig_doc_changed.emit();
  queue_draw();
  LOG_INFO("Canvas: set_refpt_as_export_origin name='{}' (cleared {} prior)",
           ref->name, cleared);
}

void Canvas::clear_refpt_export_origin() {
  if (!m_doc) {
    LOG_WARN("Canvas: clear_refpt_export_origin ignored — doc=NULL");
    return;
  }
  int cleared = 0;
  for_each_ref(m_doc, [&cleared](SceneNode *r) {
  ...
  LOG_INFO("Canvas: clear_refpt_export_origin (cleared {})", cleared);
}

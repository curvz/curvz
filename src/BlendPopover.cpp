...
BlendPopover::BlendPopover() : m_steps(SpinType::Integer) {
  ...
  m_btn_equalize.set_tooltip_text("Insert nodes into the shorter path so "
                                  ...
                                  "subdivision).");
  curvz::utils::set_name(m_btn_equalize, "pop_bld_eq",
                         "blend_popover_equalize_btn");
  m_btn_equalize.signal_clicked().connect([this]() {
    if (!m_equalize_cb)
      return;
    int new_count = m_equalize_cb();
    if (new_count <= 0)
      return; // equalize failed — leave banner as is
  ...
  // Reverse
  curvz::utils::set_name(m_reverse, "pop_bld_rv",
                         "blend_popover_reverse_check");
  ...
  curvz::utils::set_name(m_stroke_override, "pop_bld_so",
                         "blend_popover_stroke_override_check");
  ...
  curvz::utils::set_name(m_btn_cancel, "pop_bld_cnc",
                         "blend_popover_cancel_btn");
...
void BlendPopover::build_model_spins(const CanvasModel *model) {
  ...
  double start_val =
      m_last_stroke_override ? m_last_stroke_start : m_seed_a_stroke_w;
  double end_val =
...
void BlendPopover::show(Gtk::Widget &anchor, const CanvasModel *model,
...
// ── apply_with_last
// ─────────────────────────────────────────────────────────── s154 m3: Toolbar
// Blend left-click path. Two outcomes:
...
void BlendPopover::apply_with_last(Gtk::Widget &anchor,
                                   const CanvasModel *model, int a_count,
                                   int b_count, double a_stroke_w,
                                   double b_stroke_w, ApplyCb apply_cb,
                                   EqualizeCb equalize_cb) {
  if (a_count != b_count) {
...
void BlendPopover::on_ok() {
  ...
  r.stroke_w_start = m_stroke_start ? m_stroke_start->get_internal_value()
                                    : m_last_stroke_start;
  r.stroke_w_end =
      m_stroke_end ? m_stroke_end->get_internal_value() : m_last_stroke_end;
...
void BlendPopover::on_cancel() { popdown(); }

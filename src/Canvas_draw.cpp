...
// s350 m1 — the walk + PatternGlyph moved to the GlyphOutline unit so the
// SvgWriter save-presentation emit (no Canvas) shares the exact placement.
// This member is a thin wrapper supplying the project text-style library.
bool Canvas::pattern_glyph_walk(
  ...
  return Curvz::pattern_glyph_walk(
      text_obj, guide, project() ? &project()->text_styles : nullptr, fn);
}

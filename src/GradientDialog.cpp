...
static double angle_from_endpoints(double x1, double y1,
                                    double x2, double y2) {
    double dx = x2 - x1;
    double dy = y2 - y1;
...
static void endpoints_from_angle(double angle_deg,
                                  double& x1, double& y1,
                                  double& x2, double& y2) {
...
GradientDialog::GradientDialog() {
    ...
    m_actions->add_action("ctx-delete-stop",
        sigc::mem_fun(*this, &GradientDialog::on_ctx_delete_stop));
    m_actions->add_action("ctx-duplicate-stop",
        sigc::mem_fun(*this, &GradientDialog::on_ctx_duplicate_stop));
    m_actions->add_action("ctx-set-offset",
...
void GradientDialog::build_type_row() {
    ...
    curvz::utils::set_name(m_btn_linear, "dlg_gr_lin", "gradient_dialog_linear_toggle");
    curvz::utils::set_name(m_btn_radial, "dlg_gr_rad", "gradient_dialog_radial_toggle");
...
void GradientDialog::build_stop_track() {
    ...
    curvz::utils::set_name(m_track_area, "dlg_gr_trk", "gradient_dialog_track_da");
    m_track_area.set_draw_func(
        sigc::mem_fun(*this, &GradientDialog::draw_track));
    m_root.append(m_track_area);

    m_stop_btn_row.set_spacing(6);
    curvz::utils::set_name(m_btn_add_stop, "dlg_gr_as", "gradient_dialog_add_stop_btn");
    curvz::utils::set_name(m_btn_remove_stop, "dlg_gr_rs", "gradient_dialog_remove_stop_btn");
    ...
    m_btn_reverse.set_tooltip_text("Reverse stop order (flip gradient direction)");
    m_btn_distribute.set_tooltip_text("Distribute stops evenly along the gradient");
    curvz::utils::set_name(m_btn_reverse, "dlg_gr_rv", "gradient_dialog_reverse_btn");
    curvz::utils::set_name(m_btn_distribute, "dlg_gr_ds", "gradient_dialog_distribute_btn");
...
void GradientDialog::build_stop_props() {
    ...
    curvz::utils::set_name(m_opacity_spin, "dlg_gr_op", "gradient_dialog_opacity_spn");
    ...
    curvz::utils::set_name(m_color_picker, "dlg_gr_cp", "gradient_dialog_color_picker");
...
void GradientDialog::build_geom_section() {
    ...
    curvz::utils::set_name(m_angle_spin, "dlg_gr_ang", "gradient_dialog_angle_spn");

    // Knob — circular drawing area, drag to rotate.
    m_angle_knob.set_content_width(64);
    m_angle_knob.set_content_height(64);
    curvz::utils::set_name(m_angle_knob, "dlg_gr_akb", "gradient_dialog_angle_knob_da");
...
void GradientDialog::build_action_row() {
    ...
    curvz::utils::set_name(m_btn_cancel, "dlg_gr_cnc", "gradient_dialog_cancel_btn");
    curvz::utils::set_name(m_btn_apply, "dlg_gr_app", "gradient_dialog_apply_btn");
...
void GradientDialog::show(Gtk::Window& parent, FillStyle initial, Callback cb) {
    ...
    if (!initial.is_gradient()) {
        ...
        if (initial.stops.empty()) {
            GradientStop s0; s0.offset = 0.0; s0.r = 0; s0.g = 0; s0.b = 0; s0.a = 1;
            GradientStop s1; s1.offset = 1.0; s1.r = 1; s1.g = 1; s1.b = 1; s1.a = 1;
            initial.stops = { s0, s1 };
        }
        initial.g_x1 = 0.0; initial.g_y1 = 0.5;
        initial.g_x2 = 1.0; initial.g_y2 = 0.5;
        initial.g_r  = 0.5;
    }
...
void GradientDialog::draw_track(const Cairo::RefPtr<Cairo::Context>& cr,
                                int w, int h) {
    // ── Ramp strip (top) ─────────────────────────────────────────────────
...
int GradientDialog::hit_stop_square(double x, double y, int w, int /*h*/) const {
    double sy = RAMP_BOT + RAMP_GAP;
    if (y < sy - 3 || y > sy + SQUARE_H + 3) return -1;
    const double inner_w = (double)(w - 2 * GUTTER);
    ...
    for (int i = (int)m_working.stops.size() - 1; i >= 0; --i) {
        ...
        double cx = GUTTER + std::clamp(s.offset, 0.0, 1.0) * inner_w;
        double sx = cx - SQUARE_W * 0.5;
        if (x >= sx - 2 && x <= sx + SQUARE_W + 2) return i;
    }
    return -1;
}
...
void GradientDialog::on_track_press(int n_press, double x, double y) {
    ...
    if (n_press >= 2 && idx < 0) {
        ...
        if (!m_working.stops.empty()) {
            ...
            for (const auto& st : m_working.stops) {
                ...
                if (d < best_d) { best_d = d; nearest = &st; }
            }
            ns.r = nearest->r; ns.g = nearest->g;
            ns.b = nearest->b; ns.a = nearest->a;
        } else {
            ns.r = 0.5; ns.g = 0.5; ns.b = 0.5; ns.a = 1.0;
        }
        ...
        for (int i = 0; i < (int)m_working.stops.size(); ++i) {
            ...
            if (t.offset == ns.offset && t.r == ns.r && t.g == ns.g
                && t.b == ns.b && t.a == ns.a) {
                m_selected_stop = i;
                break;
            }
    ...
    if (idx < 0) return;  // single click on empty track = no-op
...
void GradientDialog::on_track_drag_update(double dx, double /*dy*/) {
    if (m_loading) return;
    if (m_track_drag_idx < 0) return;
    if (m_track_drag_idx >= (int)m_working.stops.size()) return;
    if (std::abs(dx) > 1.0) m_track_dragged = true;
...
void GradientDialog::on_knob_press(int /*n_press*/, double x, double y) {
    if (m_loading) return;
    if (m_working.type != FillStyle::Type::LinearGradient) return;
    ...
    if (deg < 0.0) deg += 360.0;
    m_angle_spin.set_value(deg);  // triggers on_angle_changed which writes endpoints
}
...
void GradientDialog::select_stop(int idx) {
    ...
    color::Color c;
    c.r = s.r; c.g = s.g; c.b = s.b; c.a = 1.0;
...
void GradientDialog::apply_type_visibility() {
    ...
    if (m_lbl_angle) m_lbl_angle->set_visible(!radial);
    // Radial-only widgets.
    m_r_spin.set_visible(radial);
    if (m_lbl_r) m_lbl_r->set_visible(radial);
}
...
void GradientDialog::on_type_changed() {
    ...
    if (was_radial == now_radial) return;  // toggle echo, no real change
    ...
    if (now_radial) {
        ...
        m_working.g_x1 = 0.5; m_working.g_y1 = 0.5;  // focal at centre
        m_working.g_x2 = 0.5; m_working.g_y2 = 0.5;  // centre at centre
        ...
        m_working.g_x1 = 0.0; m_working.g_y1 = 0.5;
        m_working.g_x2 = 1.0; m_working.g_y2 = 0.5;
    }
...
void GradientDialog::on_pos_changed() {
    if (m_loading) return;
    int n = (int)m_working.stops.size();
    if (m_selected_stop < 0 || m_selected_stop >= n) return;
...
void GradientDialog::on_add_stop() {
    ...
    if (!m_working.stops.empty()) {
        ...
        for (const auto& s : m_working.stops) {
            ...
            if (d < best_d) { best_d = d; nearest = &s; }
        }
        ns.r = nearest->r;
        ns.g = nearest->g;
        ns.b = nearest->b;
...
void GradientDialog::on_remove_stop() {
    if (m_loading) return;
    int n = (int)m_working.stops.size();
    if (n <= 1) return;  // disabled in select_stop, defensive
    if (m_selected_stop < 0 || m_selected_stop >= n) return;
...
void GradientDialog::on_distribute_stops() {
    if (m_loading) return;
    int n = (int)m_working.stops.size();
    if (n < 2) return;  // single stop has nowhere to go
...
void GradientDialog::on_reverse_stops() {
    if (m_loading) return;
    int n = (int)m_working.stops.size();
    if (n < 2) return;  // nothing to reverse
...
void GradientDialog::on_track_secondary_press(int /*n_press*/,
                                               double x, double y) {
    if (m_loading) return;
    int w = m_track_area.get_width();
    int h = m_track_area.get_height();
    int idx = hit_stop_square(x, y, w, h);
    if (idx < 0) return;  // right-click on empty track = no-op (for now)
    ...
    if (!m_track_area.translate_coordinates(m_ctx_button, x, y, bx, by)) {
        bx = 0; by = 0;  // defensive — shouldn't trigger in practice
    }
...
void GradientDialog::on_ctx_delete_stop() {
    int n = (int)m_working.stops.size();
    if (n <= 1) return;
    if (m_ctx_stop_idx < 0 || m_ctx_stop_idx >= n) return;
...
void GradientDialog::on_ctx_duplicate_stop() {
    ...
    if (m_ctx_stop_idx + 1 < n) {
        new_offset = (src.offset
                      + m_working.stops[m_ctx_stop_idx + 1].offset) * 0.5;
    } else {
...
void GradientDialog::on_angle_changed() {
    ...
    m_working.g_x1 = x1; m_working.g_y1 = y1;
    m_working.g_x2 = x2; m_working.g_y2 = y2;
...
void GradientDialog::on_apply() {
    hide();
    if (m_callback) m_callback(m_working);
}

void GradientDialog::on_cancel() {
    hide();
}

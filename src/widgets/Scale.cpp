...
Scale::Scale(std::string_view name, Glib::RefPtr<Gtk::Adjustment> adj,
...
ScriptValue Scale::invoke_leaf(std::string_view verb, const ScriptArgs &args) {
  if (verb == "set") {
    ...
    if (args[0].kind == ValueKind::Double)
      ...
      throw std::runtime_error("Scale.set expects a number");
...
std::vector<std::string> Scale::leaf_properties() const {

...
void CheckButton::bind_canonical() {
  signal_toggled().connect(
      [this]() { emit("toggled", ScriptValue::boolean(get_active())); });
}

ScriptValue CheckButton::invoke_leaf(std::string_view verb,
...
ScriptValue CheckButton::query_leaf(std::string_view property) const {
  if (property == "active")
    return ScriptValue::boolean(get_active());
  if (property == "label")
    return ScriptValue::text(get_label());
  return ScriptValue::null();
}

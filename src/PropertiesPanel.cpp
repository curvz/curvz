...
void PropertiesPanel::refresh(CanvasModel *canvas, SceneNode *obj) {
  ...
  // ── Object group ─────────────────────────────────────────────
  {
    ...
    // build_selection_section's output, matching the user's mental
    // model — Guides as a first-class object surface distinct from the
    // per-selection branches below it. The doc may be null if no
    // project is open; build_object_guides_section guards on that.
    // s354: the inspector "Text" section (the is_text() branch plus its
    // empty placeholder disclosure for non-Text selections) was removed.
    // Text editing lives in the ContextBar / TextStyleEditorDialog /
    // text context menu; a Text node now falls through to the generic
    // empty "Selection" placeholder like any non-path/non-group type.

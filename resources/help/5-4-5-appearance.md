...
The **Appearance** section is the inspector's catch-all for how a
...
node that renders has an Appearance section: paths, compound paths,
...
![The Appearance section showing binding rows, Fill paint editor, Stroke paint editor with thickness and cap/join, and the section Opacity slider](img/5-4-5-appearance-section.png)
...
- A **type-toggle** — None / Solid / currentColor / Gradient /
  Swatch — picks what kind of paint the slot carries.
...
the same controls. Below the editor are stroke-specific properties
that have no fill equivalent:
...
With more than one object selected, every change you make in
Appearance is **broadcast** to the entire selection. Picking a new
fill colour repaints them all; flipping cap from butt to round
...
recolour the selection in the Appearance editor, all five end up
with the literal new colour and no swatch binding. To keep
bindings, use the Swatches panel (6.4) to recolour the swatch
itself, not the objects.
...
- **Shadow** (5.4.6) — drop shadow, the section directly below
  Appearance for any node that supports one.
